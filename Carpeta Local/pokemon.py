import random
from base_conocimiento import efectividades, inmunidades, debilidades, habilidades, estados_pokemon, tipos_habilidad
...
class Pokemon:
    def __init__ (self, nombre, salud, habilidades, tipo, velocidad): # Constructor
        ...
        self.tipo = tipo
        self.estado = "normal"  # Estado inicial
    ...
    def CambiarEstado(self, estado):
        if estado in estados_pokemon:
            self.estado = estado
            print(f"{self.nombre} ahora está {estado}.")
    ...
    def MostrarHabilidades(self):
        for habilidad in self.habilidades:
            if habilidad in habilidades:
                ...
                print(f"  Tipo de Habilidad: {info_habilidad['tipo_habilidad']}")
                print(f"  Estado: {info_habilidad['estado']}")
                print("-" * 30)
